...
class ComputadoresController {
  ...
  static async listarComputadorPorId (req, res) {
    try {
      const id = req.params.id;
      const computadorEncontrado = await computador.findById(id);
      res.status(200).json(computadorEncontrado);
    } catch (erro) {
      res.status(500).json({ message: `${erro.message} - falha na requisição do computador` });
    }
  };
...
export default ComputadoresController;
